...
# Interval for checking file changed under /etc/kubernetes/addons.
ADDON_CHECK_INTERVAL_SEC=${TEST_ADDON_CHECK_INTERVAL_SEC:-600}

# The namespace to create addons.
...
# The business logic for whether a given object should be created
# was already enforced by ansible, and /etc/kubernetes/addons is the
# managed result is of that. Start everything below that directory.
...
# Create kubepush thirdpartyresource for kube-system.
start_addon /etc/kubernetes/addons/thirdpartyresource.yaml 100 10 "${SYSTEM_NAMESPACE}" &
