...
echo "... calling op-verify-prereqs" >&2
op-verify-prereqs
...
echo "... calling kube-downgrade" >&2
kube-downgrade

echo "... calling validate-downgrade" >&2
# Override errexit
(validate-nodes) && validate_result="$?" || validate_result="$?"
...
if [[ "${validate_result}" == "1" ]]; then
    ...
    echo "...ignoring non-fatal errors in validate-upgrade" >&2
fi
