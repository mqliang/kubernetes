...
DOMAIN_NAME_IN_DNS=${DOMAIN_NAME_IN_DNS-"NO"}
...
function aliyun-instance-prelogue-common {
  ...
  CAICLOUD_ALIYUN_CFG_STRING_CAICLOUD_CUSTOMER=${CAICLOUD_CUSTOMER}
...
function read-instance-ssh-info {
  ...
  while read line; do
    ...
    if [[ ${public_ip} == "NONE" ]] && [[ ${inner_ip} == "NONE" ]]; then
      echo "Public ip address and inner ip address are both missing" >&2
      exit 1
    fi
    if [[ ${public_ip} == "NONE" ]]; then
      public_ip="${inner_ip}"
    fi
    if [[ ${inner_ip} == "NONE" ]]; then
      inner_ip="${public_ip}"
    fi
