...
func (attacher *aliyunPersistentDiskAttacher) Attach(spec *volume.Spec, hostName string) (string, error) {
	...
	devicePath, err := attacher.aliyunVolumes.AttachDisk(hostName, volumeID, readOnly)
	if err != nil {
...
func (detacher *aliyunPersistentDiskDetacher) Detach(deviceMountPath string, hostName string) error {
	volumeID := path.Base(deviceMountPath)

	attached, err := detacher.aliyunVolumes.DiskIsAttached(hostName, volumeID)
	if err != nil {
	...
	if err = detacher.aliyunVolumes.DetachDisk(hostName, volumeID); err != nil {
